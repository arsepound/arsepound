...
- [Development guidelines](#development-guidelines)
    - [General Arsepound Core](#general-arsepound-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Arsepound Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play arsepounds" on the test network, if you
...
Arsepound Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/arsepound-project/arsepound/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_arsepound
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_arsepound --log_level=test_suite
$ valgrind -v --leak-check=full src/arsepoundd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_arsepound.coverage/index.html`.
```
...
To profile a running arsepoundd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep arsepoundd` -- sleep 60
```
...
Arsepound Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Arsepound Core, where everyone uses
...
pay attention to for reviewers of Arsepound Core code.

General Arsepound Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Arsepound Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Arsepound Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Arsepound Core so that they can be integrated
...
file descriptors it will cause problems with Arsepound's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Arsepound's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `arsepound-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `arsepound-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Arsepound Core, the wallet was always
