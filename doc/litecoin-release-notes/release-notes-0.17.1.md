Buttcoin Core version 0.17.1 is now available from:

  <https://download.buttcoin.org/buttcoin-0.17.1/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/buttcoin-project/buttcoin/issues>

To receive security and update notifications, please subscribe to:

  <https://groups.google.com/forum/#!forum/buttcoin-dev>
...
When buttcoin is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `buttcoin.conf` file, not inside included files or from
  ...
  as buttcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the buttcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=buttcoin
    test.uacomment=buttcoin-testnet
...
be used in V0.17 by starting buttcoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Buttcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Buttcoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitcointalk.org/?topic=279249) transactions.

For backend RPC convenience, the Buttcoin devs have supported to keep the acronym `PSBT`
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When buttcoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If buttcoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart buttcoind with
...
- When running buttcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If buttcoind
