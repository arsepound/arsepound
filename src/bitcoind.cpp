...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  arsepoundd [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                tfm::format(std::cerr, "Error: Command line contains unexpected token '%s', see arsepoundd -h for a list of options.\n", argv[i]);
                return false;
            }
        }
...
#endif
            tfm::format(std::cout, "Arsepound server starting\n");
